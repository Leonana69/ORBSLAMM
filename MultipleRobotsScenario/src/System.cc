    ...
    , mbUseMMapping(bUseMMapping)
{
    ...
    double duration = std::chrono::duration_cast<std::chrono::duration<double> >(t2 - t1)
                          .count();

    cout << "Vocabulary loaded in " << duration << " seconds!" << endl
         << endl;
...
void System::SetMultiMapper(MultiMapper* pMMapper)
{
    ...
    mpMMapper->AddMap(mpMap, mpKeyFrameDatabase); // Add the map to multimapper
    // once it's created - use this
    // when MM is implemented as
    // thread
    mpMMapper->SetVocabulary(mpVocabulary);
...
bool System::IsUsingMMaps() { return mbUseMMapping; }
...
void System::Shutdown()
{
    ...
    while (!mpLocalMapper->isFinished() || !mpLoopCloser->isFinished() || !mpViewer->isFinished() || mpLoopCloser->isRunningGBA() || !mpMMapper->isFinished()) {
        usleep(5000);
    }
